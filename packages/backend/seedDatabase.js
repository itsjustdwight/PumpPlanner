...
async function saveExercisesToDB() {
  try {
    ...
    console.log(
      `✅ Found ${exercises.length} exercises. Connecting to MongoDB...`
    );

    console.log(
      `🔍 Checking MONGO_CONNECTION_STRING: ${process.env.MONGO_CONNECTION_STRING}`
    ); // Debugging log
    ...
    console.log(
      "✅ Old exercises removed. Inserting new exercises..."
    );
    ...
    console.error(
      "❌ Error saving exercises to database:",
      error
    );
  } finally {
